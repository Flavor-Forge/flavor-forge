...
const AddRecipe = () => {
  const [ingredients, setIngredientFields] = useState([{ name: '', quantity: '', price: 0.0 }]);

  const addIngredientField = () => {
    setIngredientFields([ingredients, { name: '', quantity: '', price: 0.0 }]);
  };
  ...
  const submit = async (data, formRef) => {
    ...
    try {
      await Recipes.collection.insert({
        ...
        name,
        description,
        ingredients: ingredients.map(({ ingredientname, quantity, price }) => ({ ingredientname, quantity, price })),
        instructions,
        picture,
